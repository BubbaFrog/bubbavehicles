dofile(minetest.get_modpath("bubbavehicles") .. "/magicbrandon_config.lua")
...
function carpet:on_rightclick(clicker)
	...
	if self.driver and clicker == self.driver then
		...
		if bubbavehicles.one_use == true then
			self.object:remove()
		end
...
function carpet:on_punch(puncher, time_from_last_punch, tool_capabilities, direction)
	...
	if puncher and puncher:is_player() and not self.driver and not minetest.setting_getbool("creative_mode")  then
		puncher:get_inventory():add_item("main", "bubbavehicles:magic_brandon")
	end
end
...
minetest.register_entity("bubbavehicles:magic_brandon", carpet)

minetest.register_craftitem("bubbavehicles:magic_brandon", {
	description = "Magical Lets Go Brandon Flag",
	inventory_image = "magic_brandon_inv.png",
	wield_image = "magic_brandon.png",
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		minetest.add_entity(pointed_thing.under, "bubbavehicles:magic_brandon")
...
if bubbavehicles.crafts == true then
	minetest.register_craft({
        output = "bubbavehicles:magic_brandon",
        recipe = {
